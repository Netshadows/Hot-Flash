...
window.renderDashboard = function () {
    ...
    todayLogs.forEach(log => {
        ...
        // Psych Penalty
        if (log.psych?.length) penalty += (log.psych.length * 5);

        // Urogenital Penalty
        if (log.urogenital?.length) penalty += (log.urogenital.length * 5);
    });
    ...
    let chartHtml = '';
    // Generate bars representing "Impact" (inverse of resonance)
    for (let i = 0; i < 7; i++) {
        // Randomize historical data, ensure today is accurate roughly
        const height = (i === 6) ? Math.min(100, penalty * 2) : Math.floor(Math.random() * 60) + 10;
        // Color coding handled by CSS gradient mostly, but we can add classes if needed
        // For now, using same gradient for all, maybe opacity difference
    ...
    return `
        ...
        <div class="dashboard-header fade-in-up">
            <div class="dashboard-greeting">Good Morning, Jane</div>
        ...
        <div class="card chart-card fade-in-up" style="animation-delay: 0.2s;">
            <div class="chart-header">
                <span class="chart-title">Symptom Intensity</span>
